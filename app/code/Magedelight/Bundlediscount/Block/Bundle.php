...
class Bundle extends AbstractProduct
{
    ...
    \Magento\Catalog\Block\Product\Context $context, \Magento\Tax\Model\CalculationFactory $calculationFactory, \Magento\Customer\Model\Session $customerSession, \Magento\Framework\Json\EncoderInterface $jsonEncoder, array $data = []
        ...
        $bundleCollection = $this->_objectManager->create('Magedelight\Bundlediscount\Model\Bundlediscount')->getBundlesByProduct($product, $this->customerSession);

        $this->scopeConfig = $this->_objectManager->create('Magento\Framework\App\Config\ScopeConfigInterface');
        $storeScope = \Magento\Store\Model\ScopeInterface::SCOPE_STORE;
        $display_options = $this->scopeConfig->getValue('bundlediscount/others/display_options', $storeScope);

        if ($display_options == 'both') {
            $otherBundles = $this->_objectManager->create('Magedelight\Bundlediscount\Model\Bundleitems')->getCollection()
            ...
            if (count($otherIds) > 0) {
                $bundleCollection = $this->_objectManager->create('Magedelight\Bundlediscount\Model\Bundlediscount')->getBundleObjects($otherIds, $bundleCollection, $this->customerSession);
            }
        }
