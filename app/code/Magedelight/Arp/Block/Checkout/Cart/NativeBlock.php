...
class NativeBlock extends \Magento\Checkout\Block\Cart\Crosssell
{   
    const NativeEnabled = 'arp_products/shoppingcart/native/enabled';
    const slidesToShow = 'arp_products/shoppingcart/native/slidesToShow';
    const slidesToScroll = 'arp_products/shoppingcart/native/slidesToScroll';
    ...
    public function getProductBlockCrossell() {
        ...
        $relatedProductsblock = $this->collectionFactory->create()
                ...
                'eq' => \Magedelight\Arp\Model\Source\BlockPositionShoppingCart::InsteadOfNativeCrossSells
                ])
    ...
    public function getConditionsProductsIds() 
    {
        if(!empty($this->getProductBlockCrossell())){
            return $this->getProductBlockCrossell()['products_ids_conditions'];
        }
        return null;
    }
    ...
    public function setCustomTemplate() {
        ...
        if($quoteItems) {
            foreach($quoteItems as $item) {
               $productQuote[] = $item->getProductId();
            }
        } else {
               $productQuote = []; 
        }
       $conditionProductIds = explode(',' , $this->getConditionsProductsIds());
       
       $filterProducts = array_intersect($conditionProductIds, $productQuote);
       
       $override = false;
       $productoverride = $this->productFactory->create()
                        ->getCollection()
                        ->addAttributeToSelect('arp_override')
                        ->addAttributeToFilter('entity_id',['in'=>$filterProducts]);
        foreach ($productoverride as $prod) {
            if($prod->getData('arp_override')) {
                $override = true;
            }
        }            
        $blockDataCount = count($this->getProductColletion());
        
        if($blockDataCount > 0 && $override == false && $this->canShowIf()) {
            $this->setTemplate('Magedelight_Arp::checkout/cart/crossell-items.phtml');
            $this->setType('crosssell');
        }
    }
