...
class BrandSlider extends \Magento\Framework\View\Element\Template implements \Magento\Widget\Block\BlockInterface
{
    ...
    public function getItems()
    {
        ...
        $attributeCode = $this->_scopeConfig->getValue('amshopby_brand/general/attribute_code');
        if ($attributeCode == '') {
            return null;
        }

        $options = $this->_repository->get($attributeCode)->getOptions();
        array_shift($options);

        $items = [];
        foreach ($options as $option) {
            $filterCode = \Amasty\Shopby\Helper\FilterSetting::ATTR_PREFIX . $attributeCode;
            $setting = $this->_optionSettingHelper
                ->getSettingByValue($option->getValue(), $filterCode, $this->_storeManager->getStore()->getId());
    ...
    protected function _beforeToHtml()
    {
        ...
        if ($this->getData('sort_by') == 'name') {
            usort($this->_items, [$this, '_sortByName']);
        } else {
            usort($this->_items, [$this, '_sortByPosition']);
        }
