...
class Strings extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    {
        # Add empty space after each of paragraph
        $string = str_replace("</p>","</p> ", $string);

        # Strip tags
        # I use native function to make it faster
        # CPP still rules, Yeah!
        $string = strip_tags($string);

        # Decode HTML symbols to plain text
        $string = html_entity_decode($string);

        # Decode URLs to display it
        # correctly in the plain text
        $string = urldecode($string);

        # Trim spaces in the begin
