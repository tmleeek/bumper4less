    ...
    DynamicProductOptions.prototype = {
                ...
                if (this.isSystemOption(this.options[i])) {
                    ...
                    if (origField && dynamicField) {
                        ...
                        dynamicField.select('input, select, textarea').each(function(elm){
                            ...
                            if (elm.name) Event.observe(elm, 'change', this.updateQtyElement.bind(this, elm.name));
		...
		updateAllPrices: function() {
			...
			$$('#itoris_dynamicproductoptions select, #itoris_dynamicproductoptions textarea, #itoris_dynamicproductoptions input[type=text]').each(function(elm){
                ...
                if (elm.name) _this.updateQtyElement(elm.name);
			});
        ...
        updateQtyComment: function(qtyElm) {
            ...
            if (tierPrices.length > 0) {
                var tierList = "";
                var format = {"pattern":"%s","precision":2,"requiredPrecision":2,"decimalSymbol":".","groupSymbol":",","groupLength":3,"integerRequired":1};//this.getPriceBox().data('magePriceBox').options.priceConfig.priceFormat;
                for(var i=0; i<tierPrices.length; i++) {
                    tierList += '<div>Buy <b>'+tierPrices[i].qty+'</b> for <b>'+ formatCurrency(parseFloat(tierPrices[i].price), format, false) +'</b> each</div>';
                }
        ...
        getTierPrices: function(qtyElm) {
            var tierPrices = [];
            if (!qtyElm.parentElm) return tierPrices;
        ...
        var qty = parseInt(qtyElm.hasClassName('use_product_qty') ? ($('qty') ? $('qty').value : 1) : qtyElm.value), price = '', tierPrices = this.getTierPrices(qtyElm);
            if (tierPrices.length > 0) {
